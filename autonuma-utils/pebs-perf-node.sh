...
APP=$1
NUMA_NODE=$2 # NUMA node to pin to
...
if [[ ${NUM_THREADS} -lt 10 ]]; then
    PINNING="numactl -C 1-${NUM_THREADS} -m $NUMA_NODE"
elif [[ ${NUM_THREADS} -eq 10 ]]; then
    PINNING="numactl -C 1-9,20 -m $NUMA_NODE"
else
    PINNING="numactl -C 1-9,20-$((NUM_THREADS + 10)) -m $NUMA_NODE"
fi
